...
**Primary** button ideally is used only once on a page to display the most important action we want the user to take. Usually, the default size is sufficient, but to draw more attention `large` size can be used. Use `small` size sparingly—in tight spaces or more complex user interfaces.
