...
**Primary** button ideally is used only once on a page to display the most important action we want the user to take. Usually, the default size is sufficient, but to draw more attention `large` size can be used. Use `small` size sparingly - in the tight spaces or more complex user interfaces.
...
<BestPractices>
  <BestPractices.Recommended>
    ...
    - in general use default size, and other sizes as an exception
...
<BestPractices>
  <BestPractices.Recommended>
    ...
    - most of the time default size [icon](/components/icon) from Waffles is preffered, regardless of the button
    size
