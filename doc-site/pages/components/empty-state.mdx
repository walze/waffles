...
import CenteredColumn from '../../examples/empty-state/centered-column';
import Direction from '../../examples/empty-state/direction';
...
<Example title="Direction" path="empty-state/direction">
  <Direction />
</Example>

<Example title="Centered Column" path="empty-state/centered-column">
  <CenteredColumn />
</Example>
...
In addition to all custom props listed below, `EmptyState.Content` accepts all props regular `HTMLDiv` would, such as `data-` attributes.

<PropsTable
  metadata={require('!!extract-react-types-loader!../../../src/empty-state/content')}
/>
